...
void Neuron::updateInputWeights(Layer &prevLayer) {
    ...
    // in the neurosin in the preceding layer
...
Net::Net(const vector<unsigned> &topology)
{
    ...
    for (unsigned layerNum = 0; layerNum < numLayers; ++layerNum) {
        ...
        // add a bias neuron to the layers.
        for (unsigned neuronNum = 0; neuronNum <= topology[layerNum]; ++neuronNum) {
            m_layers.back().push_back(Neuron(numOutputs, neuronNum));
            cout << "Made a neuron!" << endl;
        }

        // force the bias nodes output value to 1.0. It's the last neuron created above
...
void Net::feedForward(const vector<double> &input) {
    ...
    // Assign (latch) the input values into th input neurons
...
TrainingData::TrainingData(const string filename) {
    m_trainingDataFile.open(filename.c_str(), ios_base::in);
...
void TrainingData::getTopology(vector<unsigned> &topology) {
    ...
    ss >> label;
    cout << ss << endl;
    if (this->isEof() || label.find("topology:") == string::npos) {
        cout << "Failed to find topology" << endl;
        abort();
    }
...
int main() {
    ...
    while (!trainData.isEof()) {
        ...
        // Collect the nets actual output results:
